function pagination(data, callback) {
  ...
  function pageAction(dataPage) {
    ...
    (startPage = dataPage - data.count <= 1 ? 1 : dataPage - data.count),
      (endPage =
        dataPage + data.count >= data.totalPage
          ? data.totalPage
          : dataPage + data.count),
    ...
    if (dataPage >= 5) {
      for (var i = 1; i <= 2; i++) {
        i_html +=
          '<a data-page="' + i + '" href="javascript:void(0);">' + i + "</a>";
      }
      i_html += "<span>...</span>";
    }
    for (var j = startPage; j <= endPage; j++) {
      i_html +=
        '<a data-page="' + j + '" href="javascript:void(0);">' + j + "</a>";
    }
    if (endPage + 1 < data.totalPage) {
      i_html += "<span>...</span>";
      for (
        var i =
          endPage > data.totalPage - 2 ? data.totalPage : data.totalPage - 1;
        i <= data.totalPage;
        i++
      ) {
        i_html +=
    ...
    if (data.jumpBtn && data.totalPage >= 1) {
      i_html +=
        'Goto<input id="pageInput" type="text" />Page<span id="inputGo">aa</span>';
    }
  ...
  page.onclick = function (event) {
    ...
    if (target.id == "inputGo") {
      var pageInput = document.getElementById("pageInput"),
        goPage =
          pageInput.value > data.totalPage
            ? 1
            : /[1-9]+/g.test(pageInput.value)
            ? pageInput.value
            : 1;
